...
import RectangleFigure, { headlandSummary } from './RectangleFigure';
  ...
  return (
    <View style={styles.field}>
      ...
      <TextInput
        ...
        // Highlight the whole existing value on focus so a tap-and-type
        // replaces it instead of the caret landing mid-number.
        selectTextOnFocus
      />
    </View>
  );
...
function RectanglePreview({ rectangle }: { rectangle: RectangleDefinition }) {
  return (
    <View style={styles.preview}>
      <Text style={styles.previewTitle}>Rectangle preview</Text>
      <Text style={styles.body}>
        {rectangle.mFt.toFixed(1)} ft along M × {rectangle.nFt.toFixed(1)} ft across N · coverage {rectangle.side.toUpperCase()}
      </Text>
      <RectangleFigure definition={rectangle} />
      <Text style={styles.help}>{headlandSummary(rectangle)}</Text>
    </View>
  );
}
...
export default function SetupWizard(props: SetupWizardProps) {
  ...
  return (
    <View style={styles.screen}>
      {/* Fixed header: STOP never scrolls, everything else sits under it. */}
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.content}>
        ...
        {state.phase === 'rectangle' ? (
          <View style={styles.card}>
            ...
            <Text style={styles.help}>The rover turns outside the rectangle. The clear pavement its turns need is worked out for you and shown in the preview — keep that area clear.</Text>
            {state.rectangle ? <RectanglePreview rectangle={state.rectangle} /> : null}
        ...
        {['readiness', 'arming', 'armed', 'starting'].includes(state.phase) ? (
          <View style={styles.card}>
            ...
            <Text style={styles.check}>{state.calibrationStatus === 'ready' ? '✓' : state.wet ? '✕' : '△'} Calibration {state.calibrationStatus}{state.calibrationStatus !== 'ready' ? ' — open Diagnostics to calibrate' : ''}</Text>
            ...
            {state.rectangle ? (
              <Text style={styles.check}>{state.rectangle.headlandSource === 'rover' ? '✓' : '△'} {headlandSummary(state.rectangle)}</Text>
            ) : null}
      ...
      <Modal visible={showDiagnostics} animationType="slide" onRequestClose={() => setShowDiagnostics(false)}>
        <View style={styles.screen}>
          <View style={styles.header}>
            ...
            <Text style={styles.status}>Only needed when the mount, steering, tires, load, phone, or firmware changes — the stored calibration carries between runs.</Text>
          </View>
          <ScrollView style={styles.scroll} contentContainerStyle={styles.content}>
            <View style={styles.card}>
...
const styles = StyleSheet.create({
  ...
  preview: { borderColor: '#496274', borderWidth: 1, borderRadius: 8, padding: 10, gap: 8 },
